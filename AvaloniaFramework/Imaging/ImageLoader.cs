...
public static class ImageLoader
{
    ...
    /// </summary>
    /// <remarks>
    /// A virtualizing panel recycles a row's controls onto a different item as it scrolls. Without
    /// this, a slow decode started for the row that just scrolled away would arrive and paint that
    /// item's photo onto the row now showing another one.
    /// </remarks>
    ...
    private static async void Refresh(Image image)
    {
        ...
        // Blank while the decode runs, so a recycled row never shows the previous item's photo
...
#pragma warning restore CA1031
        {
            ...
            // full-size decode running out of memory on a phone, most plausibly. A row with no
